...
def parse_args():
    ...
    parser.add_argument('--warmup_way', type=str, default="sup",
                        help='uns, sup')
    ...
    parser.add_argument('--trainval_root', default='./dataset/webvision-50/',
                        help='root for trainval data')
    parser.add_argument('--val_root', default='./dataset/imagenet/',
    ...
    parser.add_argument('--network', type=str, default='RN18',
    ...
    parser.add_argument(
        ...
        help=('Approximation for numerical stability taken from'
    ...
    for epoch in range(args.initial_epoch, args.epoch + 1):
        ...
        if (epoch % 10 == 0) or (epoch == args.epoch):
            ...
            if acc >= args.best_acc:
                ...
                best_acc5 = acc5
            print(
             'KNN top-1 precion: {:.4f} {:.4f}, best is: {:.4f} {:.4f}'.format(
                acc*100.,
                acc5*100.,
                args.best_acc*100.,
                best_acc5*100
        ...
        if (epoch % 10 == 0):
            ...
            torch.save(
                ...
                os.path.join(
                    exp_path,
                    snapLast + '_'+str(epoch)+'epoch.pth'
                )
            )
            torch.save(
                ...
                os.path.join(
                    exp_path,
                    snapLast + '_ema_'+str(epoch)+'epoch.pth'
                )
            )
            torch.save(
                ...
                os.path.join(
                    exp_path,
                    'scheduler_'+str(epoch)+'epoch.pth'
                )
            )
            torch.save(
                uns_contrast,
                os.path.join(
                    exp_path,
                    'uns_contrast_'+str(epoch)+'epoch.pth'
                )
            )
            torch.save(
                queue,
        ...
        if (epoch > args.warmup_epoch) and (
