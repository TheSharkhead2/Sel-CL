...
def unsupervised_masks_estimation(args, queue, mix_index1, mix_index2, epoch, bsz, device):
    # If no labels used, label is the index in mini-batch
    labelsUnsup = torch.arange(bsz).long().unsqueeze(1).to(device)
    ...
    if args.sup_queue_use == 1 and epoch > args.sup_queue_begin:
        ...
        maskUnsup_mem = torch.zeros((2 * bsz, queue.K)).float().to(device)
    ...
    # Minor label in 1st mini-batch part equal to mayor label in the first mini-batch part (note that mayor label of 1st and 2nd is the same as we force the original image to always be the dominant)
    quad1_unsup = torch.eq(labelsUnsup[mix_index1], labelsUnsup.t()).float()
    ...
    quad3_unsup = torch.eq(labelsUnsup[mix_index2], labelsUnsup.t()).float()
    ...
    if args.sup_queue_use == 1 and epoch > args.sup_queue_begin:
        ...
        # Mini-batch samples with memory samples (add columns)
        mask2Unsup_mem = torch.zeros((2 * bsz, queue.K)).float().to(device)

        if queue.ptr == 0:
            mask2Unsup_mem[:, -2 * bsz:] = mask2Unsup_batch
        else:
            mask2Unsup_mem[:, queue.ptr -
                           (2 * bsz):queue.ptr] = mask2Unsup_batch
...
def supervised_masks_estimation(args, index, queue, queue_index, mix_index1, mix_index2, epoch, bsz, device, features, selected_pair_th, noisy_labels, selected_examples):
    ...
    if (selected_pair_th == -1):
        temp_graph = torch.eq(noisy_labels[index].unsqueeze(
            1), noisy_labels[index].unsqueeze(1).t())
    else:
        similarity = torch.mm(features[index], features[index].t())
        noisy_pairs = torch.eq(noisy_labels[index].unsqueeze(
            1), noisy_labels[index].unsqueeze(1).t())
        temp = torch.zeros(len(index), len(index)).type(torch.uint8)
        temp_graph = torch.where(similarity < selected_pair_th, temp, noisy_pairs.type(
            torch.uint8)).type(torch.bool)
    ...
    if args.sup_queue_use == 1 and epoch > args.sup_queue_begin:
        ...
        if (selected_pair_th == -1):
            temp_graph_mem = torch.eq(noisy_labels[index].unsqueeze(
                1), noisy_labels[queue_index].unsqueeze(1).t())
        else:
            similarity = torch.mm(features[index], features[queue_index].t())
            noisy_pairs = torch.eq(noisy_labels[index].unsqueeze(
                1), noisy_labels[queue_index].unsqueeze(1).t())
            temp = torch.zeros(len(index), len(queue_index)).type(torch.uint8)
            temp_graph_mem = torch.where(
                similarity < selected_pair_th, temp, noisy_pairs.type(torch.uint8)).type(torch.bool)

            selected_index1 = selected_examples[index].nonzero().reshape(-1)
            selected_index2 = selected_examples[queue_index].nonzero(
            ).reshape(-1)
    ...
    if args.sup_queue_use == 1 and epoch > args.sup_queue_begin:
        ...
        # Mini-batch samples with memory samples (add columns)
        mask2Sup_mem = torch.cat(
            (maskExtended_sup3_1, maskExtended_sup3_2), dim=0)
