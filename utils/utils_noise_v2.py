...
def train_sel(args, scheduler, model, model_ema, contrast, queue, device, train_loader, train_selected_loader, optimizer, epoch, features, selected_pair_th, selected_examples):
    ...
    for batch_idx, (img, labels, index) in enumerate(train_loader):
        ...
        img1, img2, labels, index = img[0].to(device), img[1].to(
            device), labels.to(device), index.to(device)
        ...
        img1, y_a1, y_b1, mix_index1, lam1 = mix_data_lab(
            img1, labels, args.alpha_m, device)
        img2, y_a2, y_b2, mix_index2, lam2 = mix_data_lab(
            img2, labels, args.alpha_m, device)
        ...
        if args.sup_queue_use == 1:
            queue.enqueue_dequeue(torch.cat((embedA_ema.detach(), embedB_ema.detach()), dim=0), torch.cat((predsA_ema.detach(
            ), predsB_ema.detach()), dim=0), torch.cat((index.detach().squeeze(), index.detach().squeeze()), dim=0))

        if args.sup_queue_use == 1 and epoch > args.sup_queue_begin:
            queue_feats, queue_pros, queue_index = queue.get()

        else:
            queue_feats, queue_pros, queue_index = torch.Tensor(
                []), torch.Tensor([]), torch.Tensor([])

        maskUnsup_batch, maskUnsup_mem, mask2Unsup_batch, mask2Unsup_mem = unsupervised_masks_estimation(
            args, queue, mix_index1, mix_index2, epoch, bsz, device)
        ...
        if args.sup_queue_use == 1 and epoch > args.sup_queue_begin:
            ...
            pairwise_comp_mem = torch.matmul(
                embeds_mem[:2 * bsz], embeds_mem[2 * bsz:].t())
            pros_simi_mem = torch.mm(
                pros_mem[:2 * bsz], pros_mem[2 * bsz:].t())

        maskSup_batch, maskSup_mem, mask2Sup_batch, mask2Sup_mem = \
            supervised_masks_estimation(args, index.long(), queue, queue_index.long(
            ), mix_index1, mix_index2, epoch, bsz, device, features, selected_pair_th, noisy_labels, selected_examples)

        # Negatives mask, i.e. all except self-contrast sample
        logits_mask_batch = (torch.ones_like(
            maskSup_batch) - torch.eye(2 * bsz).to(device))

        loss_sup = Supervised_ContrastiveLearning_loss(args, pairwise_comp_batch, maskSup_batch, mask2Sup_batch,
                                                       maskUnsup_batch, mask2Unsup_batch, logits_mask_batch, lam1, lam2, bsz, epoch, device, batch_idx)

        # compute simi_loss
        loss_simi = Simi_loss(args, pros_simi_batch, maskSup_batch, mask2Sup_batch, maskUnsup_batch,
                              mask2Unsup_batch, logits_mask_batch, lam1, lam2, bsz, epoch, device, batch_idx)
        ...
        if args.sup_queue_use == 1 and epoch > args.sup_queue_begin:
            ...
            if queue.ptr == 0:
                ...
                logits_mask_mem[:, queue.ptr -
                                (2 * bsz):queue.ptr] = logits_mask_batch

            loss_mem = Supervised_ContrastiveLearning_loss(
                args, pairwise_comp_mem, maskSup_mem, mask2Sup_mem, maskUnsup_mem, mask2Unsup_mem, logits_mask_mem, lam1, lam2, bsz, epoch, device, batch_idx)

            loss_sup = loss_sup + loss_mem

            loss_simi_mem = Simi_loss(args, pros_simi_mem, maskSup_mem, mask2Sup_mem, maskUnsup_mem,
                                      mask2Unsup_mem, logits_mask_mem, lam1, lam2, bsz, epoch, device, batch_idx)
            loss_simi = loss_simi + loss_simi_mem

            sel_mask = (maskSup_batch[:bsz].sum(
                1)+maskSup_mem[:bsz].sum(1)) < 2
        else:
        ...
        try:
            ...
            train_selected_loader_iter = iter(train_selected_loader)
            img, labels, _ = next(train_selected_loader_iter)
        img1, img2,  labels = img[0].to(
            device), img[1].to(device), labels.to(device)
        ...
        counter = counter + 1
    print('train_sel_loss', train_loss_1.avg, 'train_simi_loss',
          train_loss_2.avg, 'train_class_loss', train_loss_3.avg)
...
def train_sup(
    ...
    for batch_idx, (img, labels, index) in enumerate(train_loader):
        ...
        img1, y_a1, y_b1, mix_index1, lam1 = mix_data_lab(
            img1, labels, 0, device)
        img2, y_a2, y_b2, mix_index2, lam2 = mix_data_lab(
            img2, labels, 0, device)
        ...
        maskSup_batch, maskSup_mem, mask2Sup_batch, mask2Sup_mem = \
            supervised_masks_estimation(args, index.long(), queue, queue_index.long(
            ), mix_index1, mix_index2, epoch, bsz, device, None, -1, noisy_labels, None)
        ...
        if counter % 15 == 0:
            print('Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}, Learning rate: {:.6f}'.format(
                ...
                optimizer.param_groups[0]['lr']))
        counter = counter + 1
    print('train_sup_loss', train_loss_1.avg,
...
def pair_selection(args, net, device, trainloader, testloader, epoch, features):
    ...
    temploader = torch.utils.data.DataLoader(
        trainloader.dataset, batch_size=args.test_batch_size, shuffle=False, num_workers=8)
    ...
    soft_labels = torch.zeros(len(labels), args.num_classes).scatter_(
        1, labels.view(-1, 1), 1)

    D, I = index.search(features_numpy, args.k_val+1)
    neighbors = torch.LongTensor(I)
    # weight is calculated by embeddings' similarity
    weights = torch.exp(torch.Tensor(D[:, 1:])/args.sup_t)
    N = features_numpy.shape[0]
    score = torch.zeros(N, args.num_classes)

    for n in range(N):
        neighbor_labels = soft_labels[neighbors[n, 1:]]
        score[n] = (neighbor_labels*weights[n].unsqueeze(-1)
                    ).sum(0)  # aggregate labels from neighbors
    pseudo_labels = torch.max(score, -1)[1]
    soft_labels = torch.zeros(len(pseudo_labels), args.num_classes).scatter_(
        1, pseudo_labels.view(-1, 1), 1)
    ...
    agreement_measure = (torch.max(soft_labels, dim=1)[
                         1] == labels).float().data.cpu()
    ...
    for i in range(args.num_classes):
        ...
        top_clean_class_relative_idx = torch.topk(
            discrepancy_class, k=int(k_corrected), largest=False, sorted=False)[1]
    ...
    for i in range(args.num_classes):
        idx_class = temploader.dataset.targets == i
        idx_class = torch.from_numpy(idx_class.astype("float")) == 1.0
        idx_class = (selected_examples.type(torch.bool)
                     & idx_class).nonzero().squeeze()
        ...
        temp_similarities = torch.sum(
            class_features.repeat(500, 1)*random_class_features, -1)
        if (i == 0):
