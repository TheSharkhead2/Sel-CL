...
from losses import (
    Supervised_ContrastiveLearning_loss,
    ClassificationLoss,
    Simi_loss
)
import time
    ...
    for batch_idx, (img, labels, index) in enumerate(train_loader):
        ...
        with amp.autocast():
            ...
            if args.sup_queue_use == 1 and epoch > args.sup_queue_begin:
                ...
                sel_mask = (
                    maskSup_batch[:bsz].sum(1)+maskSup_mem[:bsz].sum(1)
                ) < 2
            else:
        ...
        with amp.autocast():
            ...
            loss = (
                sel_loss + args.lambda_c * lossClassif +
...
def train_sup(
    ...
    for batch_idx, (img, labels, index) in enumerate(train_loader):
        ...
        try:
            ...
            img, labels, _ = next(train_selected_loader_iter)
        img1, img2,  labels = (
            img[0].to(device), img[1].to(device), labels.to(device)
        )
...
def pair_selection(args, net, device, trainloader, testloader, epoch):
    ...
    trainFeatures = torch.rand(
        len(trainloader.dataset), args.low_dim).t().to(device)
    ...
    with torch.no_grad():
        for batch_idx, (inputs, _, _) in enumerate(temploader):
            ...
            trainFeatures[
                :,
                batch_idx * batchSize:batch_idx * batchSize + batchSize
            ] = features.data.t()
    ...
    with torch.no_grad():
        ...
        for batch_idx, (inputs, targets, index) in enumerate(temploader):
            ...
            # the index yi (from top-k operation)
            retrieval = torch.gather(candidates, 1, yi)

            retrieval_one_hot_train.resize_(batchSize * args.k_val, C).zero_()

            # Generate the K*batchSize one-hot encodings from neighboring
            ...
            probs_norm = (
                probs_corrected / torch.sum(probs_corrected, dim=1)[:, None]
            )
    ...
    with torch.no_grad():
        ...
        for batch_idx, (inputs, targets, index) in enumerate(temploader):
            ...
            prob_temp = probs_norm[
                torch.arange(0, batchSize), train_new_labels[index]]
            prob_temp[prob_temp <= 1e-2] = 1e-2
            prob_temp[prob_temp > (1 - 1e-2)] = 1 - 1e-2
            discrepancy_measure2_pseudo_labels[index] = -torch.log(prob_temp)

            agreement_measure[index.data.cpu()] = (
                torch.max(probs_norm, dim=1)[1] == targets).float().data.cpu()
